/// `TestOption` is an enumeration of the available test options that can be
/// selected in the skill tree view.
///
...
/// An extension on the `TestOption` enum to provide a string representation for
/// each test option.
///
/// This extension adds a `description` getter on `TestOption` to easily retrieve
/// the human-readable string associated with each option. This is particularly
/// helpful for UI display purposes.
extension TestOptionExtension on TestOption {
  /// Gets the string representation of the test option.
  ///
  /// Returns a human-readable string that describes the test option. This string
  /// is intended to be displayed in the UI for user selection.
  String get description {
    switch (this) {
      ...
      /// In case of a test suite option that includes selected node and ancestors,
      ...
      /// In case of an option to run all tests in a category, return the corresponding
      /// string.
      case TestOption.runAllTestsInCategory:
        return 'Run all tests in category';

      /// In case of an undefined or unknown test option, return a generic unknown
      /// string. This case should ideally never be hit if all enum values are handled.
      default:
        return 'Unknown';
    }
  }
